...
hospital = Hospital("Sethu", "Hospital")
...
with st.sidebar:
    st.title("AI Healthcare Montoring ")
    page = option_menu(
        menu_title='Menu',
        options=['Doctor', "Nurse", "Patient", "Operation","Treatment"],
    ...
    # Operation Page
      ...
      with header:
        st.title('Treatment Section 👨‍⚕️'.title())
        ...
        if st.button("Learn More About Treatment"):
            st.markdown(
                f'<a href="https://ai-based-healthcare.streamlit.app" target="_blank" class="highlight">Click here to explore Treatment Details</a>',
                unsafe_allow_html=True
            )
            

        # Add a visualization (example graph of AI-based treatment effectiveness)
        st.write("### Treatment Effectiveness by Condition")
        
        # Sample Data for AI-based Treatment Effectiveness
        data = {
            'Condition': ['Diabetes', 'Heart Disease', 'Cancer', 'Parkinson', 'Chronic Disease'],
            'Effectiveness (%)': [85, 78, 72, 80, 90]
        }
        df = pd.DataFrame(data)

        # Plotting the Bar Chart
        fig = px.bar(df, x='Condition', y='Effectiveness (%)', color='Condition', title="AI Treatment Effectiveness by Condition",
                     labels={'Effectiveness (%)': 'Treatment Effectiveness (%)', 'Condition': 'Health Condition'})
        fig.update_layout(
            plot_bgcolor="white",
        ...
        st.markdown("""
        <p class="small-font">
        The graph above shows the effectiveness of AI-driven treatments across different health conditions. As AI technology evolves, it becomes increasingly proficient in providing personalized treatments that are not only effective but also aligned with the latest medical standards.
        </p>
        """, unsafe_allow_html=True)
